...
* Start with Ubuntu 20.04 image
	  ...
	  git clone git@github.com:janoside/stats.cool.git
	  cd stats.cool
	  npm i
	  pm2 start bin/main.js --name stats
	  
	  wget "https://raw.githubusercontent.com/janoside/stats.cool/master/docs/nginx-config.txt"
	  mv nginx-config.txt /etc/nginx/sites-available/stats.cool
